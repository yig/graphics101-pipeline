...
void main()
{
    ...
    vec3 tangent_space_normal = 2*texture( object_space_normal_map, fTexCoord ).rgb - 1;
    vec3 object_space_normal = normalize( tangent_space_to_object_space * tangent_space_normal );
    
    // Color is the normal (packed into the range [0,1]).
    FragColor = vec4( object_space_normal*0.5 + 0.5, 1.0 );
}
